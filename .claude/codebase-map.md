...
| `src/logging/lifecycle.rs` | Single owner of the `LogGuard` (R-A4-001): `LogLifecycle` serializes `clear` (non-owning `LogHandle` flush + clear-write slot) with `exit` (one bounded final shutdown, `ExitOutcome`); `OwnedGuard` trait seam for fake-guard tests |
