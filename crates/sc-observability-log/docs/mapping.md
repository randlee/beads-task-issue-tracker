...
| `helpers` | `HelperHealth` (QA-2 RSH-004): `flush_in_flight` is the `FLUSH_IN_FLIGHT` flag, set while a flush helper runs (claimed by `flush_installed`, cleared by a drop guard when sc-observability's flush returns or unwinds); `detached` is `DETACHED_HELPERS`, incremented by `run_bounded` when a caller's timeout elapses with the helper still running and decremented by the helper's exit guard when its work finishes (flush and shutdown helpers together). Plain atomics: no lock on the flush or emit path. The bridge emits no record for these transitions (reentrancy) |
...
| `InitError::IdentityResolution { source: sc_observability_types::IdentityError }` | `init` | `SC_OBSERVABILITY_LOG_IDENTITY_RESOLUTION_FAILED` (the wrapped source's code; `resolve_identity` builds it for both paths) | a failing `ProcessIdentityPolicy::Resolver` (its error kept as the source), or a `ProcessIdentityPolicy::Auto` hostname lookup that failed (the `io::Error` kept as the source) or returned an empty hostname; `INSTALLED` is reset | the wrapped source's remediation. Resolver: fix the `ProcessIdentityResolver`, or use `Auto`/`Fixed`, then call `init` again. `Auto`: ensure the operating system resolves a non-empty hostname, or use `Fixed`, then call `init` again. Tests: `identity_resolver_ok_and_err`, `identity_auto_fails_when_the_hostname_lookup_fails`, `identity_auto_fails_on_an_empty_hostname`, `init_error_codes_and_remediations` |
...
| `FlushError::InProgress` | `LogGuard::flush`, `LogControl::flush` | `SC_OBSERVABILITY_LOG_FLUSH_IN_PROGRESS` | a previous flush helper is still running (concurrently, or detached after its caller's `TimedOut`); checked after the slot, so `ShutDown` wins once shutdown started. No helper thread is started and nothing new is flushed; `helpers.flush_in_flight` is true | recoverable: wait until `helpers.flush_in_flight` is false, then retry; a flush that never finishes points at a stuck sink or disk |
...
| `BoundedError::{TimedOut, Spawn { source: std::io::Error }, WorkerLost}` (crate-private) | `run_bounded` | — | mapped 1:1 into the `FlushError`/`ShutdownError` variants above; `TimedOut` counts the helper in `helpers.detached` until its work finishes. If the work finishes exactly at the deadline (the helper publishes its value and completes after the caller's `recv_timeout` timed out but before its RUNNING→DETACHED CAS), the caller takes the value with `try_recv` instead of `TimedOut` and uncounts the helper. Test: `run_bounded_losing_race_returns_the_published_value` | — |
...
| `failure` | `Failure`, internally tagged by `operation` (`init` / `flush` / `shutdown` / `submit`) and then by `kind` (`InitFailure`, `FlushFailure`, `ShutdownFailure`, or `SubmitError` itself, whose `InvalidInput` adds a `reason` tag). Timeouts carry `timeout_ms`. `FlushFailure` kinds: `timed_out`, `logger`, `helper_spawn`, `helper_lost`, `shut_down`, `in_progress`. |
