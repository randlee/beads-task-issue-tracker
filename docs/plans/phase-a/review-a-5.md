...
Verified by the QA manager; fixed with commit title `fix(a-5): PR52 QA-1 …`.

| id | severity | finding | disposition |
| --- | --- | --- | --- |
| ATM-QA-101 | Blocking | R-A5-006's losing-race branch in `run_bounded_in` (failed RUNNING→DETACHED CAS, then `try_recv`) had no test: the detached-helper tests hold the helper past the timeout, so the CAS always succeeds. | fixed — `run_bounded_in` takes an `after_timeout` hook that runs only after `recv_timeout` timed out and before the CAS (production `run_bounded` passes a no-op; no behavior change). Unit test `run_bounded_losing_race_returns_the_published_value` releases the work in the hook and waits for `HELPER_DONE`, then asserts `Ok(value)` and a detached counter of 0; it fails when the branch returns `TimedOut`. Cited in `docs/mapping.md`'s `BoundedError` row and the R-A5-006 row. |
| ARCH-001 / ATM-QA-102 | Important | `README.md` "Process identity" still said `Auto` records the pid with no hostname. | fixed — README states that `Auto` resolves a non-empty OS hostname plus the PID once at init and that a failed or empty hostname fails `init` with `InitError::IdentityResolution`. Plan docs keep their history with a "superseded by R-A5-008" note (`sprint-a-1.md` Process identity and Out of Scope, `sprint-a-5.md` review items). |
| RBP-F001 | Important | `InitError::IdentityResolution::remediation()` always said to fix the resolver or use `Auto`/`Fixed`, which is wrong for an `Auto` hostname failure. | fixed — `code()` and `remediation()` delegate to the wrapped source's diagnostic, like `InitError::Logger`. `resolve_identity` builds both paths with the stable `SC_OBSERVABILITY_LOG_IDENTITY_RESOLUTION_FAILED` (the `Auto` path previously used sc-observability-types' `SC_OBSERVABILITY_TYPES_IDENTITY_RESOLUTION_FAILED` internally): the resolver path wraps the resolver's error as source with "fix the ProcessIdentityResolver / or use Auto or Fixed", the `Auto` path says "ensure the OS resolves a non-empty hostname / or use Fixed". Tests: `init_error_codes_and_remediations`, `identity_resolver_ok_and_err`. `api_freeze` unaffected. |
| RBP-F002 | Minor | `docs/mapping.md` inventory row for `InitError::IdentityResolution` listed only the resolver cause and remediation. | fixed — Cause lists a failing `Resolver` and an `Auto` hostname lookup failure or empty hostname; Remediation matches RBP-F001. |
| RBP-F003 | Minor | `ShutdownStep::StillShared` was unreachable: the shutdown helper called `take_sole(installed, None)`, which never returns `None`. | fixed — `take_sole(Arc<T>) -> T` has no deadline (the deadline form had no production caller and was removed with its test); `ShutdownStep` is gone and the helper returns sc-observability's `FlushError` directly. Test: `take_sole_succeeds_once_clone_is_released`. |
| QA-001 | Minor | R-A5-008's failure branches in `resolve_identity` were untested. | fixed — crate-private `resolve_auto_identity(hostname_of)` seam; production passes `hostname::get`, public API unchanged. Tests: `identity_auto_fails_when_the_hostname_lookup_fails`, `identity_auto_fails_on_an_empty_hostname`. |
| QA-002 | — | Pre-existing `src-tauri` clippy / xwin warnings in `updates.rs` and `attachments.rs`. | deferred to issue #49. |
