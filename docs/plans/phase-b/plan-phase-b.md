...
- **phase-a merged to `develop`.** Done: PR #56 merged `integrate/phase-a` to `develop` at `94e44d3` (2026-09-14). a-6 (publishing `sc-observability-log` from `../sc-observability`) has not run and `docs/plans/phase-a/handoff-a-6.md` does not exist; phase-b does not wait for it (next bullet).
- **`integrate/phase-b` exists, created from `develop@94e44d3`.** Every sprint's `target:` is `integrate/phase-b`; every sprint branch is created from it.
- **`sc-observability-log` stays in-tree (maintainer decision 2026-09-14; resolves OQ-3).** The three phase-a crates at `crates/sc-observability-log`, `crates/sc-observability-log-macros`, `crates/sc-observability-log-consumer-check` remain in the repository as members of the root workspace, at their own version `0.1.0` (`crates/Cargo.toml:10`), unchanged in source. `btit-app` keeps the path dependency (`src-tauri/Cargo.toml:38`, `sc-observability-log = { path = "../crates/sc-observability-log" }`) through `[workspace.dependencies]`. Completing a-6 and publishing the crate is still required, but the refactor does not depend on it: switching `btit-app` to the published crate and deleting the in-tree copies is follow-up work after a-6 ("Not part of phase-b").
...
- **Re-baselining at b-1 start.** This plan cites `src-tauri/src/*` line numbers at `a18c724`; the implementation baseline is `develop@94e44d3` (`IMPLEMENTATION_BASELINE=94e44d3`). phase-a's a-5 (PRs #50, #52, merged in #56) changed `logging.rs` (bridge lifecycle, `LogControl`, a `logging/lifecycle.rs` module), so `logging.rs` cites are stale and other files may have shifted. b-1's first task records `implementation_baseline: develop@94e44d3` in its Implementation Notes and re-verifies every `file:line` cite this plan and the sprint docs make against that commit, correcting them in the sprint docs' Implementation Notes (not in this plan). Cites into `crates/Cargo.toml`, `.github/workflows/ci.yml` and `scripts/check_version_sync.py` below were taken at `94e44d3`.

## Binding outcomes

### Workspace layout (maintainer requirement 1)

- **One workspace, root manifest.** `Cargo.toml` at the repository root holds `[workspace]`, `[workspace.package]`, `[workspace.dependencies]` and `[workspace.lints]`. Members are `crates/btit-app`, `crates/btit-types`, `crates/btit-beads`, `crates/btit-cli`, `crates/btit-bd`, `crates/btit-br` and the three phase-a crates `crates/sc-observability-log`, `crates/sc-observability-log-macros`, `crates/sc-observability-log-consumer-check` (in-tree, frozen; see below). One `Cargo.lock` at the root, one `target/` at the root.
- **Why not `crates/Cargo.toml`; the phase-a crates join the root workspace.** The transitional phase-a workspace at `crates/Cargo.toml` (`[workspace.package] version = "0.1.0"`, `edition = "2024"`, `rust-version = "1.94.1"`, `crates/Cargo.toml:9-13`) cannot be the btit workspace: `[workspace.package].version` must be the app version (`1.24.5`, `src-tauri/Cargo.toml:9`). Cargo also resolves a package's workspace by walking up to the nearest `Cargo.toml` with `[workspace]`, so a second workspace nested under `crates/` would claim any new `crates/btit-*` crate. b-1 therefore merges the phase-a workspace into the root one: `crates/Cargo.toml` and `crates/Cargo.lock` are deleted; its `[workspace.dependencies]` (`crates/Cargo.toml:15-30`, including the `sc-observability-log-macros = { version = "=0.1.0", path = … }` pin at `:30`) and `[workspace.lints]` (`:32-43`) move into the root manifest; the three crates become root members. Because the root `[workspace.package]` carries the app's `version`/`edition`/`rust-version`, the three crates stop inheriting those four keys (`version.workspace = true`, `edition.workspace = true`, `rust-version.workspace = true`, `license.workspace = true`, e.g. `crates/sc-observability-log/Cargo.toml:3-6`) and carry them explicitly: `version = "0.1.0"`, `edition = "2024"`, `rust-version = "1.94.1"`, `license = "MIT"` — the same values, so nothing about the crates changes but the spelling of four manifest keys. That `[package]` edit in b-1 is the only phase-b edit to `crates/sc-observability-log*/**`; their `src/`, `tests/`, `docs/`, `clippy.toml` and `README.md` are frozen for the phase (ownership table). The phase-a gates keep running from the root in the `crates` CI job (`ci.yml:92-153`, rewritten by b-1): `tests/api_freeze.rs` and the other integration tests via `cargo test -p sc-observability-log`, the runtime-dependency diff `cargo tree --locked -p sc-observability-log -e normal --target all --prefix none --format '{p}' | sed -E 's/ \(.*$//' | LC_ALL=C sort -u | diff - crates/runtime-deps.txt` (`ci.yml:132`, minus `--manifest-path`), the S1 test-isolation loop scoped to `crates/sc-observability-log*/tests/*.rs` (`ci.yml:138` globs `crates/*/tests/*.rs`, which from b-2 would also sweep `btit-*` tests), rustdoc `-D missing-docs` (`ci.yml:144-145`), the consumer-check crate (`cargo test -p sc-observability-log-consumer-check`) and the MSRV check (`ci.yml:150-153`) as `cargo +1.94.1 check --locked -p sc-observability-log -p sc-observability-log-macros -p sc-observability-log-consumer-check --all-targets` (the `btit-*` members require 1.98.1 and are not selected; Cargo enforces `rust-version` only for packages it compiles). `crates/runtime-deps.txt` is regenerated by b-1 only if the merged lockfile moves a version; the crate-name set must not change (b-1 AC).
- **Version single source of truth (ADR-003).** `[workspace.package].version` in the root `Cargo.toml`. Every `btit-*` crate and the app use `version.workspace = true`. `tauri.conf.json` keeps no `version`. The three `sc-observability-log*` crates keep their own version `0.1.0` and are **not** bumped with the app. `scripts/check_version_sync.py` today requires every workspace member manifest to use `version.workspace = true` (`check_version_sync.py:71-75`) and every local package in the lockfile to carry the workspace version (`:86-95`), so both checks would fail on the three crates; b-1 updates the script to read the root manifest, root lockfile and `crates/btit-app/tauri.conf.json`, and adds an explicit exemption set `INDEPENDENT_VERSION_MEMBERS = {"sc-observability-log", "sc-observability-log-macros", "sc-observability-log-consumer-check"}` that (a) is skipped by the `version.workspace = true` check and instead must carry one identical explicit `version`, equal to the `=0.1.0` pin in `[workspace.dependencies]`, and (b) is skipped by the lockfile check. `--set` never touches them. The OK line names them separately (b-1 Deliverable 9). Every other check stays as it is (`check_version_sync.py:62-118`).
- **Toolchain (ADR-004).** `[workspace.package].rust-version = "1.98.1"` equals the `rust-toolchain.toml` channel; the script enforces it (`check_version_sync.py:99-107`). The three `sc-observability-log*` crates keep `rust-version = "1.94.1"` explicitly (their MSRV), and the phase-a MSRV step (`ci.yml:150-153`, `cargo +1.94.1`, installed with `rustup` rather than a workflow `toolchain:` key for the reason at `ci.yml:147-148`) stays in the `crates` job, scoped to those three packages.
- **Edition.** Open question OQ-1. Sprint docs are written for `edition = "2021"` at the workspace level (today's app edition, `src-tauri/Cargo.toml:13`), so every move is code motion that the existing 142 tests pin; the `sc-observability-log*` crates keep `edition = "2024"` explicitly, unaffected by OQ-1. `resolver = "2"` (today's app value, `src-tauri/Cargo.toml:6`) is kept at the root; the phase-a workspace's `resolver = "3"` (`crates/Cargo.toml:7`) differs only in enabling the MSRV-aware resolver by default, which the `--locked` lockfile makes moot. Switching to 2024 is a maintainer decision; if taken before b-1 starts, b-1 sets it and each crate-creating sprint runs `cargo fix --edition` as part of its move.
...
- **CI.** The `backend` job runs `cargo check --workspace --all-targets` and `cargo test --workspace` from the root (`ci.yml:87,90` today point at `src-tauri/Cargo.toml`), which now also compiles and tests the three in-tree `sc-observability-log*` crates; `Swatinem/rust-cache` `workspaces: src-tauri` (`ci.yml:84`) and `workspaces: crates` (`ci.yml:113`) are removed (default `.`). The `crates` job keeps its name and its phase-a gates, re-pointed at the root with `-p` package selection (above). A `rust-quality` job (fmt, clippy `-D warnings`) covers the new crates from b-2 and the app crate from b-12.
...
Design headroom, not planned work: `ProjectRef` is a `#[non_exhaustive]` enum with the single variant `Local { cwd: Option<String> }` (today's `cwd: Option<&str>`); a remote transport adds a variant and callers that only pass a `ProjectRef` through do not change. The traits are synchronous, matching today's blocking `Command::output()` calls inside `async` Tauri commands (`cli.rs:555-560`); an async twin is not planned. The consequence — a `#[tauri::command] async fn` calling the synchronous backend blocks the async runtime for the child's lifetime — is today's behaviour and is kept; moving the calls to `spawn_blocking` at the `btit-app` command boundary is deferred to issue #55 (plan QA RSH-002).
...
**Dolt mode: one implementation, enum switch (maintainer decision 2026-09-15).** Embedded and server Dolt are one `btit-bd` implementation in phase-b. b-10 adds `DoltMode { Embedded, Server, ProxiedServer }` resolved from `metadata.json` with bd's `GetDoltMode()` rules; any mode-dependent behaviour is a `match` on it, not a second trait or implementation. Splitting into separate embedded and server implementations (e.g. behind a Dolt admin accessor for a server settings / database listing view) is deferred to a later phase and decided then.
...
- **No panics; deny lint set.** The root `[workspace.lints]` is phase-a's set (`crates/Cargo.toml:32-43`): `pedantic` warn plus `unwrap_used`, `expect_used`, `panic`, `unreachable`, `todo`, `unimplemented`, `indexing_slicing` = `deny`. `btit-types`, `btit-beads`, `btit-cli`, `btit-bd`, `btit-br` inherit it from creation (`[lints] workspace = true`, per-crate `clippy.toml` `allow-*-in-tests`). Moved code is fixed where it violates: `parts[0]`/`parts[1]`/`parts[2]` in `parse_bd_version` (`cli.rs:314-317`), `.lock().unwrap()` on `CLI_CLIENT_INFO` and `BD_PROJECT_LOCKS` (`cli.rs:327,519,548,553`). `btit-app` adopts the lint set in b-12 (`logging.rs` already carries the file-level `#![deny(..)]` from a-4).
- **Crates do not wrap `sc-observability-log`.** Only `btit-app` depends on `sc-observability-log` (`install_logging`, `logging.rs:88-110`); the in-tree crate is a workspace member, but no `btit-*` library crate lists it (per-crate `cargo tree --depth 1` gates in b-2..b-6 list their dependencies exhaustively, and b-3/b-4 grep `sc_observability` out of `src/`). Library crates depend on the `log` facade only; `btit-beads::logging` gates calls, it does not wrap the bridge.
...
flowchart BT
  ...
  app["btit-app (package beads-issue-tracker)<br/>Tauri commands, slot Arc&lt;dyn BeadsBackend&gt;, fs features<br/>+ tauri, reqwest, notify, dirs"]
  scl["sc-observability-log 0.1.0 (in-tree, frozen in phase-b)<br/>+ sc-observability-log-macros (=0.1.0), consumer-check"]
  app --> scl
  beads --> types
  cli --> beads
  cli --> types
...
Dependency rules (checked by each sprint's `cargo tree` gate): `btit-types` has no workspace dependency; `btit-beads` depends only on `btit-types`; `btit-cli` on `btit-types`, `btit-beads`; `btit-bd` and `btit-br` on those three and never on each other; only `btit-app` depends on `tauri` and `sc-observability-log` (the in-tree `sc-observability-log-consumer-check` also depends on `sc-observability-log`, `crates/sc-observability-log-consumer-check/Cargo.toml:11`; it is not a `btit-*` crate and the gates filter on `^(btit-|beads-issue-tracker)`).
...
| --- | --- | --- | --- | --- | --- | --- |
| `b-1` | `feature/sprint-b-1-workspace-foundation` | `phase-b-core` · 1 | phase-a merged; `integrate/phase-b` exists | none | [`sprint-b-1.md`](./sprint-b-1.md) | root workspace; Tauri crate at `crates/btit-app`; phase-a crates merged into the root workspace as in-tree members (0.1.0, path dependency kept); CI, release, version script, docs paths updated; app builds, 142 app tests and the phase-a crate tests pass |
...
| Relation | Rationale |
| --- | --- |
| `b-1 must_follow phase-a merge` | b-1 merges the phase-a `crates/` workspace into the root workspace and moves `src-tauri/`; it needs the phase-a crates and `logging.rs` as merged by PR #56 (`develop@94e44d3`). a-6 is not a prerequisite (OQ-3 resolved: in-tree). |
...
| `src-tauri/**` | **moves** to `crates/btit-app/**` | — | — | — | — | — | — | — | — | — | — | — |
| `crates/Cargo.toml`, `crates/Cargo.lock` | **deletes** (merged into the root manifest and lockfile) | — | — | — | — | — | — | — | — | — | — | — |
| `crates/sc-observability-log*/Cargo.toml` `[package]` (`version`, `edition`, `rust-version`, `license` made explicit) | **owns** | — | — | — | — | — | — | — | — | — | — | — |
| `crates/sc-observability-log*/{src,tests,docs}/**`, `clippy.toml`, `README.md` | frozen (no phase-b sprint edits them) | — | — | — | — | — | — | — | — | — | — | — |
| `crates/runtime-deps.txt` | regenerated only if the merged lockfile moves a version (names unchanged) | — | — | — | — | — | — | — | — | — | — | — |
| `.github/workflows/ci.yml` | **backend job; `crates` job re-pointed at the root** | adds `rust-quality` | extends | extends for `btit-cli`, `btit-bd`, `btit-br` | — | — | — | — | — | — | — | workspace-wide fmt/clippy |
...
- The three `sc-observability-log*` crates are root workspace members at `0.1.0` with their source unchanged from `develop@94e44d3`: `git diff --stat 94e44d3...HEAD -- 'crates/sc-observability-log*/src' 'crates/sc-observability-log*/tests' 'crates/sc-observability-log*/docs' 'crates/sc-observability-log*/clippy.toml'` is empty; `btit-app` is the only `btit-*`/app package depending on `sc-observability-log` (b-7 Acceptance Criterion 4 gate); the `crates` CI job is green.
...
- Switching `btit-app` from the in-tree path dependency to the published `sc-observability-log` (crates.io version or git `rev`, as a-6 will record in `docs/plans/phase-a/handoff-a-6.md`) and deleting the in-tree copies `crates/sc-observability-log`, `crates/sc-observability-log-macros`, `crates/sc-observability-log-consumer-check`, `crates/runtime-deps.txt`, the `crates` CI job and the `check_version_sync.py` exemption: follow-up work after a-6 publishes (OQ-3, maintainer decision 2026-09-14).
...
| OQ-3 | ~~`sc-observability-log` dependency in b-1: crates.io version or git `rev`?~~ **Resolved 2026-09-14 (maintainer):** in-tree path dependency for all of phase-b; the three phase-a crates join the root workspace unchanged at `0.1.0`. The switch to the published crate is follow-up work after a-6 ("Not part of phase-b"). | in-tree path dependency |
