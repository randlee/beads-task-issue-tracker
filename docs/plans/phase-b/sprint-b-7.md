...
1. **Backend slot** (`backend.rs`, code sample): `static SLOT: RwLock<Option<Arc<dyn CliBackend>>>`, `static PROJECT_LOCKS: LazyLock<Arc<ProjectLocks>>` (one lock map for the process, so replacing the backend never allows two concurrent `bd` processes on one project), `install(binary)`, `replace(binary)`, `current() -> Arc<dyn CliBackend>`. `current()` before `install` builds a backend for `"bd"` (today's `CLI_BINARY` default, `config.rs:8`). Every lock access recovers poisoning with `PoisonError::into_inner`.
