...
4. **`check_bd_compatibility`** (moved to `backend.rs`, same command name and `CompatibilityInfo` shape). The rule, as code, preserving `cli.rs:622-661`:
   ...
   if let (Some(p), Some(_)) = (&fresh, tuple) {
       ...
       if slot_now != Some((p.client, p.version)) { install_with(&binary, Some(p.clone())); }   // build_backend_with(binary, Some(fresh))
   }
...
pub(crate) fn install(binary: &str) { install_with(binary, probe_cli_binary(binary)) }
