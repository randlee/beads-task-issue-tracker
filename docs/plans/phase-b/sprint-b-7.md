...
4. **`check_bd_compatibility`** (moved to `backend.rs`, same command name and `CompatibilityInfo` shape): fresh `probe_cli_binary`; when `(probe.client, probe.version)` differs from the slot's `cli().map(|c| (c.client(), c.version()))` the slot is rebuilt (today's cache refresh, `cli.rs:643-646`); the four capability fields (`supports_daemon_flag`, `uses_jsonl_files`, `uses_dolt_backend`, `supports_list_all_flag`; `CompatibilityInfo` has no `supports_delete_hard_flag`, `cli.rs:612-615`) come from `capabilities_for(client, tuple)` on the fresh probe, exactly as today's wrappers read the just-refreshed cache; `legacy`, `min_supported_major`, `searched_paths`, `warnings` unchanged.
...
6. **Shims for b-8.** `cli.rs` keeps exactly four `pub(crate)` fns (`project_uses_dolt(beads_dir)`, `supports_daemon_flag`, `uses_jsonl_files`, `client_info`) delegating to the slot. `polling.rs` (`get_beads_mtime`), `watcher.rs` and `fs_commands.rs` compile unchanged against them; `migration.rs` changes only at its import line and the two `get_cli_client_info()` matches (`:4,500-510,596-605`, rewritten to `client_info()` with the semantics stated in Exact Targets); `updates.rs:1-3` keeps `use crate::cli::{detect_cli_client, parse_bd_version}`. Nothing else remains in `cli.rs`. The `project_uses_dolt(beads_dir: &Path)` shim keeps today's path-based signature and turns it into `ProjectRef::local(Some(<beads_dir's parent>))` (every caller passes `<working_dir>/.beads`); b-8 replaces the callers with `ProjectRef`s directly.
...
fn build_backend_with(binary: &str, probe: Option<CliProbe>) -> Arc<dyn BeadsBackend> {
    match probe {
        Some(p) if p.client == CliClient::Br && p.version.is_some() => Arc::new(btit_br::BrCli::new(binary, PROJECT_LOCKS.clone())),
        _ => Arc::new(btit_bd::BdCli::new(binary, PROJECT_LOCKS.clone())),   // Bd, Unknown, Br without a parsed version, or no answer (today's `_ =>` arms; cli.rs:361-364)
    }
}
