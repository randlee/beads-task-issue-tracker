...
- `.github/workflows/ci.yml`: `rust-quality` becomes workspace-wide (`cargo fmt --check --all`, `cargo clippy --workspace --all-targets --all-features -- -D warnings`); the `backend` job unchanged
