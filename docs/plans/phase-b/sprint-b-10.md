...
8. **`.claude/codebase-map.md`.** The architecture box names the six crates; `## Backend Structure` is rewritten per crate (files, purpose, line counts at the sprint head), the Tauri command tables keep their 65 rows, "Key Backend Patterns" 1-3 describe the backend slot, `ProjectLocks` and `BeadsBackend::sync`, and "Global State (Rust)" lists `SLOT`, `PROJECT_LOCKS`, `LOGGING_ENABLED`/`VERBOSE_LOGGING` (in `btit-beads`), `LAST_SYNC_TIME`, `LAST_KNOWN_MTIME`, `PROBE_CHILD`, watcher state.
