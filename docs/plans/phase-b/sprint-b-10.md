...
3. No test in `btit-bd` spawns a CLI, checked mechanically on every OS: `! grep -rnE --exclude=api_freeze.rs 'BdCli::new\(|BdCli::with_seeded_probe\(|Command::new|probe_cli_binary|probe_version_output' crates/btit-bd/tests` (`api_freeze.rs` excluded: it is byte-frozen by deliverable 8 and only constructs a `BdCli` without probing — corrected at team-lead triage, 2026-09-15) and, for in-file `#[cfg(test)]` modules, `awk '/#\[cfg\(test\)\]/{t=1} t&&/(BdCli::new\(|Command::new|probe_cli_binary|probe_version_output)/{print FILENAME":"FNR": "$0}' crates/btit-bd/src/*.rs` prints nothing. On Linux and macOS only, additionally `SAFE_PATH="$(dirname "$(command -v cargo)"):/usr/bin:/bin"; ! PATH="$SAFE_PATH" command -v bd && PATH="$SAFE_PATH" cargo test -p btit-bd` passes with no `bd` reachable (skipped on Windows, where the greps are the gate).
