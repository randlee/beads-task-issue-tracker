...
3. **`sc-observability-log` from outside btit.** `[workspace.dependencies] sc-observability-log = "<version from handoff-a-6.md>"` (or the git `rev` form, OQ-3). `crates/btit-app/src/logging.rs` (and any `logging/` submodule a-5 introduced) compiles unchanged against it: the API names btit uses are those recorded in `docs/plans/phase-a/handoff-a-6.md` for the published crates (the a-5 fix PRs #50/#52 change the bridge lifecycle, so the `logging.rs:26-28` import list at `a18c724` is not authoritative). The developer records the actual import list from the re-baselined `logging.rs` in this doc's Implementation Notes. If the published API differs from what btit imports, that is a hard stop, not a local patch.
