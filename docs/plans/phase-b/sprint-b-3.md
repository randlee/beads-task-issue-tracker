...
9. **Contract doc.** `crates/btit-beads/docs/backend-contract.md`: the trait inventory (from the plan, with signatures), the error table, the `ProjectRef` headroom note, the "sync trait, blocking calls" note, and the rule that only `btit-app` depends on `tauri` and `sc-observability-log`.
...
1. `cargo tree -e normal -p btit-beads --depth 1` lists exactly `btit-types`, `log`, `serde_json`; `! grep -rnE 'std::(process|fs)|tauri|sc_observability' crates/btit-beads/src`.
