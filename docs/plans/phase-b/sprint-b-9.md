...
- B11: `priority_to_number` now accepts `p`/`P` case-insensitively (`"P1"` → `"1"`); any digit outside 0-4 (`p5`-`p9`, which bd's `ParsePriority` rejects) falls back to `"3"`, same as any other unparseable input.
